...
export const billingApi = {
  ...
  getStatus: (token) => apiFetch('/api/billing/status', { token }),
  verifySession: (sessionId, token) =>
    apiFetch(`/api/billing/verify?session_id=${encodeURIComponent(sessionId)}`, { token }),
  createCheckout: (plan, token) =>
  ...
  createPortal: (token) =>
    apiFetch('/api/billing/portal', { method: 'POST', token }),
};
...
export const premiumApi = {
  getSettings: (token) => apiFetch('/api/users/me/premium', { token }),
  updateSettings: (body, token) =>
    apiFetch('/api/users/me/premium', { method: 'PATCH', body: JSON.stringify(body), token }),
  boostProfile: (token) =>
    apiFetch('/api/users/me/premium/boost', { method: 'POST', token }),
  uploadGalleryPhoto: (file, token) => {
    const formData = new FormData();
    formData.append('photo', file);
    return apiUpload('/api/users/me/premium/gallery', formData, token);
  },
  removeGalleryPhoto: (photoUrl, token) =>
    apiFetch('/api/users/me/premium/gallery', {
      method: 'DELETE',
...
export const featureApi = {
  concierge: (message) =>
    fetch('/api/concierge', {
      ...
      if (!res.ok) throw new Error(data.message || data.error || 'Concierge request failed');
      return data;
    }),
